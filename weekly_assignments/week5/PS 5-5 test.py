...
class TestBookkeeping:
    ...
    def test_q4_as_lower_inventory_std_than_symmetric(self):
        """A-S optimal inventory std ≤ symmetric baseline (core paper claim)."""
        np.random.seed(5)
        df   = _make_q4_df(n=300)
        opt  = _q4_run_backtest(df, _q4_as_quotes,        0.1, 0.001, 1.5, 0.001)
        base = _q4_run_backtest(df, _q4_symmetric_quotes,  0.1, 0.001, 1.5, 0.001)
        m_opt  = _q4_compute_metrics(opt)
        m_base = _q4_compute_metrics(base)
        assert m_opt['std_inventory'] <= m_base['std_inventory'] + 1e-9
...
get_ipython().system('pytest test_ps5_q5.py -v')
